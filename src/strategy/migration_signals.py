...
def _eval_s3(d: dict):
    ...
    if yoy is None and beat is None:
        missing.append("net_profit_yoy/consensus_beat")
    if ret20 is None:
        missing.append("ret_20d")
    if missing:
        return None, {}, False, missing
    earnings_ok = bool(beat) if beat is not None else (yoy is not None and yoy > S3_YOY_PROXY)
    ev = {"net_profit_yoy": None if yoy is None else round(yoy, 4),
          "consensus_beat": beat, "ret_20d": round(ret20, 4), "score": ret20,
          "rule": f"业绩超预期(或YoY>{S3_YOY_PROXY:.0%}) AND ret20 黄>{S3_RET20_YELLOW:.0%} 红>{S3_RET20_RED:.0%}; 事件型不迟滞"}
    level = None
