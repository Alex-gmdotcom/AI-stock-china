...
# ── marker: REDLINE_FIX_OPINION_TIER_V1 (I1.3 三级来源修订案,2026-07-03 已批) ──
def _load_openclaw_bundle(ticker: str, max_age_hours: int = 24):
    """Tier-1: openclaw 港股新闻 JSON。
    路径 ~/.ai-hedge-fund/hk_news/{ticker}_{YYYYMMDD}.json(I3.2),≤24h 有效。
    读不到/过期/schema 不符 → None(降级 Tier-2 自动新闻源)。"""
    try:
        import glob
        import os as _os
        import time as _time
        base = _os.path.join(_os.path.expanduser("~"), ".ai-hedge-fund", "hk_news")
        cands = sorted(glob.glob(_os.path.join(base, f"{ticker}_*.json")), reverse=True)
        if not cands:
            return None
        newest = cands[0]
        if (_time.time() - _os.path.getmtime(newest)) > max_age_hours * 3600:
            return None
...
class SentimentAssessment(BaseModel):
    ...
    def _coerce(cls, data):
        ...
        if not d.get("key_narratives"):
            # 消除 key_narratives=[] 与 reasoning 讲叙事的自相矛盾:显式占位
            d["key_narratives"] = ["无明确主导叙事(模型未单列,详见 reasoning)"]
        return d
...
（sentiment / risks / summary 等一律不接受）。必须包含且仅包含以下字段：
{
  ...
  "key_narratives": ["驱动情绪的核心叙事，最多3条；不得为空数组，无明确叙事时填'无明确主导叙事'"],
  "black_swan": {
    "detected": true或false,
...
def china_public_opinion_agent(
    ...
    for ticker in tickers:
        # Step 2: 三级来源(I1.3) — Tier-1: openclaw JSON 优先
        progress.update_status(agent_id, ticker, "Checking openclaw bundle")
        _oc = _load_openclaw_bundle(ticker)
        if _oc is not None:
            source_tier = "openclaw"
            _items = (list(_oc.get("news") or [])
                      + list(_oc.get("announcements") or [])
                      + list(_oc.get("risk_events") or []))
            stock_headlines = []
            for _it in _items[:30]:
                if isinstance(_it, dict):
                    _t = _it.get("title") or _it.get("description") or _it.get("summary") or ""
                    _s = _it.get("source") or "openclaw"
                    if _t:
                        stock_headlines.append(f"[{_s}] {_t}")
        else:
            # Tier-2: 自动新闻源 — 可给信号,但必须标注来源(修订案核心)
            source_tier = "auto_news"
        ...
        if not all_headlines:
            ...
            opinion_analysis[ticker] = {
                ...
                "confidence": 20,
                "reasoning": {
                    "summary": ("【数据缺口】无 openclaw 数据且自动新闻源为空,"
                                "强制中性(I1.3 Tier-3)。建议导入 openclaw JSON。"),
                    "source_tier": "none",
                    "data_sources": 0,
        ...
        reasoning = {
            ...
            "source_note": ("openclaw 结构化输入(Tier-1)" if source_tier == "openclaw"
                            else "自动新闻源(Tier-2,非 openclaw);港股建议导入 openclaw JSON"),
            "data_sources": {
