...
def china_capital_flow_agent(
    ...
    # Step 1: Market-wide northbound flow
    progress.update_status(agent_id, None, "Fetching northbound capital flow")
    northbound_data = get_northbound_flow(limit=30)

    # Analyze northbound trend
    nb_signal, nb_confidence, nb_reasoning = _analyze_northbound(northbound_data)
    ...
    for ticker in tickers:
        ...
        # Northbound 35% + Margin 25% + Stock-specific 40%
        ...
        for signal, conf, weight in [
            (nb_signal, nb_confidence, 0.35),
            (margin_signal, margin_confidence, 0.25),
            (stock_signal, stock_confidence, 0.40),
        ]:
