...
try:
    ...
    try:
        from utils.i18n import agent_name_zh, localize_reasoning  # type: ignore
    except ImportError:
        def agent_name_zh(a): return (a or "").replace("_agent", "").replace("_", " ")
        def localize_reasoning(t): return str(t)
...
def _collect(ticker: str, analyst_signals: dict) -> list[dict]:
    ...
    for agent, by_ticker in (analyst_signals or {}).items():
        ...
        if not isinstance(s, dict):
            continue
        rows.append({
            "agent": agent_name_zh(agent),
            "signal": str(s.get("signal", "neutral")).lower(),
            "confidence": float(s.get("confidence") or 0),
            "reasoning": localize_reasoning(_norm_reasoning(s.get("reasoning"))),
        })
    return rows
