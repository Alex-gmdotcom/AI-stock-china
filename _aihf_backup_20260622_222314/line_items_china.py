...
v2.0.0 换地基:A 股从同花顺三表(py_mini_racer/V8 多线程硬崩)迁到 Baostock。
  ...
  - ⚠️ Baostock 无现金流量表绝对科目 → capital_expenditure /
    depreciation_and_amortization / free_cash_flow /
    dividends_and_other_cash_distributions 一律 None(诚实留空,
...
def _a_share_by_period(norm: str, end_date: str, limit: int) -> dict:
    """A 股 → {report_period: {line_item_field: value}}(Baostock)。"""
    if _bsd is None or not _bsd.available():
        return {}
    quarters = _bsd.get_quarters(norm, end_date or "9999-12-31", limit=limit)
    by_period: dict[str, dict] = {}
    for blk in quarters:
