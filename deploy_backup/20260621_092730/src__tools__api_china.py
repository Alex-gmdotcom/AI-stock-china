...
def _get_prices_df_with_fallback(norm: str, market: str, ak_start: str, ak_end: str):
    ...
    海外 IP / 系统代理拦截东财时（push2his.eastmoney.com ProxyError），自动回退
    到新浪源（doctor 实测 stock_zh_a_daily / stock_hk_daily 海外可达）。
    ...
    def _sina():
        if market == "HK":
            ...
            # stock_hk_daily 无日期参数，返回全历史，按区间裁剪
            if df is not None and len(df) and "date" in getattr(df, "columns", []):
                ...
                df = df[(ds >= s) & (ds <= e)]
            return df
        return _ak.stock_zh_a_daily(symbol=_to_sina_code(norm), start_date=ak_start,
                                    end_date=ak_end, adjust="qfq")

    if market not in ("HK", "SH", "SZ", "BJ"):
        return None

    for name, fn in (("eastmoney", _em), ("sina", _sina)):
        try:
            df = fn()
        except Exception as exc:
